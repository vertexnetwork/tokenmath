...
GPT-4.1 is flat-priced: **$2 per 1M input tokens, $8 per 1M output**, with no tier surcharge no matter how much context you push through. Its context window is **1,047,576 tokens** — not the 128K figure some calculators still cite from the model's launch. The calculator above will warn you if your input would exceed that window, but at typical usage there's plenty of headroom.

<ModelPricingTable modelId="gpt-4-1" />

## Worked examples

Real numbers at the scale people actually search for — from a short turn up to a near-full-context call. Paste your own prompt above to price an exact input; there's no tier surcharge, so cost scales linearly (unlike Gemini 2.5 Pro).
...
<WorkedExamples
  ...
  scenarios={[
    ...
    {
      label: "100K-token input",
      inputTokens: 100000,
      outputTokens: 2000,
      description: "A large document or codebase slice with a structured response.",
    },
    {
      label: "400K-token input",
      inputTokens: 400000,
      outputTokens: 4000,
      description: "A multi-file repo or long transcript scored in a single call.",
    },
    {
      label: "1M-token input",
      inputTokens: 1000000,
      outputTokens: 8000,
      description: "Near-full-context — a whole codebase or document collection.",
    },
  ]}
/>
